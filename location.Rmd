...
#  geom_polygon(data = world_map, aes(x = long, y = lat, group = group), fill = "lightgray", color = #"white") +
  ...
  labs(title = "Spatial Distribution - Area", x = "Longitude", y = "Latitude")+
  theme_minimal()
```

### Distribution of serious levels and premises types

The plot on the left shows what the distribution of the serious level of crimes in LA looks like, and the right one shows the distribution of crimes premises type.
...
crime_plotly = crime_type |>
  ...
  arrange(area_name)
p1 <- ggplot(crime_plotly, aes(x = lon, y = lat, color = as.character(part_1_2))) +
  ...
  theme(legend.position = 'bottom',
        legend.text = element_text(size = 6))+
  labs(title = "Spatial Distribution - Level and Premises", x = "Longitude", y = "Latitude", color = 'Serious Level')
p2 <- ggplot(crime_plotly, aes(x = lon, y = lat, color = Col1)) +
  geom_point(alpha = 0.4, size = 0.3) +
  scale_fill_viridis_c() +
  theme_minimal() +
  scale_color_discrete()+
  theme(legend.position = 'none')+
  labs(x = "Longitude", y = "Latitude", color = 'Premises Type')
p1+p2
```

### Crime level difference among spatial distribution
```{r, message=FALSE}
ggplot(crime_lat, aes(x = lon, y = lat)) +
  stat_density_2d(aes(fill = after_stat(level)), geom = "polygon") +
  scale_fill_viridis_c() +
  theme_minimal() +
  labs(title = "Spatial Heatmap", x = "Longitude", y = "Latitude")
```
...
buttons_grouped <- list(
  list(type = "buttons", direction = "down", x = -0.3, y = 1.2, buttons = buttons[1:11]),
  list(type = "buttons", direction = "down", x = -0.7, y = 1.2, buttons = buttons[12:21])
)
